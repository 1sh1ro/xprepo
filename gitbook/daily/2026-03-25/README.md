...
- BTC 主导率 N/A。
- Top10 资产广度统计不完整。
- 衍生品：BTC/ETH 资金费率分别为 +0.15bps / +0.00bps，DVOL 收盘 51.21 / 73.19。
...
- BTC：$70,646.38（24h +0.91%，区间 $68,923.07 - $72,026.09，当前位于区间 56%）=> 区间震荡。
- ETH：$2,154.80（24h +0.74%，区间 $2,103.02 - $2,199.02，当前位于区间 54%）=> 区间震荡。
...
- 利用率最高样本：Aave-USDC（Ethereum） 75.38%，Borrow APY 3.33%。
- 奖励收益池数量：1 个。当前收益主体仍以原生利率为主。

稳定币收益对比表（运营参考）
| 币种 | Aave Total APY | 对比协议 Total APY | 差值(Aave-对比) | Aave Utilization |
|---|---:|---:|---:|---:|
| USDC | 2.23% | Spark-Ethereum 4.14% | -1.91% | 75.38% |
| USDT | 1.91% | Spark-Ethereum 3.44% | -1.53% | 69.98% |
| USDS | 0.08% | Spark-Ethereum 2.55% | -2.47% | 1.94% |
| PYUSD | 2.04% | Spark-Ethereum 3.75% | -1.71% | 60.43% |
...
- Funding 最高样本：Binance-BTC，年化约 4.17%。
- Funding 最低样本：OKX-ETH，年化约 -5.76%。
...
截至 2026-03-25，全市场市值 N/A，24h 成交额 N/A，BTC 主导率 N/A。
...
资金费率（Funding）仍在中性附近，BTC/ETH 分别 +0.15bps / +0.00bps；未平仓合约（OI）为 $956.25M / $286.04M；隐含波动率指数（DVOL）位于 Neutral（中性波动定价） / Panic（高波动溢价）。
资金费率接近中性，说明方向拥挤度有限；但 DVOL 仍偏高，市场对突发波动仍保留保险溢价。因此更合适的做法不是激进追单边，而是围绕波动管理仓位和节奏。

![情绪与波动当日快照](charts/chart_sentiment_snapshot.png)

恐惧与贪婪指数（F&G）当日 14（较前日 +3）；配合 BTC/ETH DVOL 51.21/73.19，当前更像情绪修复中的高波动区。
...
- CoinGecko Top资产数据获取失败（已按 key 尝试 demo）: demo: <urlopen error _ssl.c:1112: The handshake operation timed out>
- CMC 交易所报价数据获取失败: <urlopen error _ssl.c:1112: The handshake operation timed out>
