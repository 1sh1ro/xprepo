...
## 快速结论
- 全市场聚合指标较前一观测日上升：市值 +4.56%，成交额 +14.46%，当前规模分别为 $2.47T 和 $129.80B。
- BTC 主导率 59.28%（+0.47 个百分点），Top10 外市值占比（集中度口径）3.72%。
- 头部风险资产（排除稳定币、质押及信用映射）上涨 7 / 下跌 0 / 平盘 0，平均涨跌幅 +5.64%，首尾分化 16.30 个百分点。
- 前排交易所样本 7 家下跌、3 家上涨；受 Upbit +50.19% 等极端值影响，24h 变化均值仍为 +0.60%，平台间分化明显。
- 代币化股票（Tokenized Stocks）类别规模 $2.46B，7D +1.87%（最近一次可得类别快照）。

## 市场主线
今日市场状态为“交易性修复”：全市场聚合市值与成交额共振上行，但前排交易所样本多数成交走弱；现有样本仅覆盖头部风险资产，尚不足以判断长尾市场是否扩散或新一轮趋势是否启动。

前排交易所样本仅支持成交收缩及降幅分化，不支持资金回流判断。Deribit BTC/ETH 8h 资金费率分别为 +1.32bps / +1.60bps，接近中性；近期 OKX 清算样本中多头清算额高于空头，但不足以单独确认杠杆拥挤。BTC 隐含波动率处于低波动定价区间，ETH 处于中性区间；单凭隐含波动率指数（DVOL）仍不足以判断期权保护成本是否便宜。情绪回到偏乐观区，若成交不跟随，需防止短线追高回撤。上述指标共同用于描述市场状态，但暂不足以归因于特定事件。

## BTC/ETH 24h 趋势
![BTC/ETH 24h价格路径](charts/chart_btc_eth_24h_trend.png)

口径：文字涨跌来自交易所滚动 24 小时行情；图内路径为当前可得的 23 个小时点，首尾变化 BTC +6.79%、ETH +3.90%，两者窗口不可混用。
- BTC（交易所滚动 24 小时口径）：$76,718.00（+6.62%，区间 $71,132.00 - $79,500.00，当前位于区间 67%）=> 偏强，上行主导。
- ETH（交易所滚动 24 小时口径）：$2,371.45（+3.14%，区间 $2,255.68 - $2,447.98，当前位于区间 60%）=> 偏强，上行主导。
- 简评：BTC 与 ETH 24h 表现同步偏强，但当前窗口尚不足以确认上行动能延续。
...
价格与成交同步上行，当前可描述为修复型反弹；若次日成交继续配合，可作为修复延续的观察条件，仍不能据此确认趋势启动。
...
清算口径：仅为近期 OKX 公共清算样本，不代表 OKX 或全市场清算总量；25Δ 偏度定义为 Put IV 减 Call IV。
...
计算口径：以当前可得 23 个小时点的高低点作为支撑/阻力，触发缓冲取现价 0.1% 与 0.25×ATR14 的较大值；触发价用于观察确认，不是自动交易指令。

## 今日差异化重点
### 稳定币收益与资金成本
- 收益端：在扩展协议样本中，Compound-USDC 的总年化收益率（Total APY）为 6.46%，其中包含约 0.11% 奖励收益；需继续区分原生利率与奖励补贴。
- 资金需求：本期可得样本最高利用率 91.91%；高利用率意味着借款需求较强，也可能压缩退出流动性。
- 跨平台成本：本期可得报价中，USDC 借币年化样本最高约 4.44%、最低约 2.14%，按未四舍五入值相差约 2.29 个百分点；仍需核对额度、期限和地区准入，不直接视为套利利差。
...
2. 若衍生品占比继续上升而 Deribit 8h Funding 仍接近中性，只能说明该口径下成交结构更偏向衍生品；是否意味着杠杆拥挤或放大波动，仍需结合未平仓合约、DVOL 与成交验证。
