...
def main() -> int:
    parser = argparse.ArgumentParser(description="Call ASCN API")
    parser.add_argument("path", help="Endpoint path, e.g. /health")
    parser.add_argument("--method", choices=["GET", "POST"], default="GET")
    parser.add_argument("--base-url", default=os.environ.get("ASCN_BASE_URL", "https://api.ascn.ai"))
    ...
    api_key = os.environ.get("ASCN_API_KEY")
    if not api_key:
        print("ASCN_API_KEY is required", file=sys.stderr)
        return 2

    api_key_header = os.environ.get("ASCN_API_KEY_HEADER", "x-api-key")
