#!/usr/bin/env python3
"""Configurable ChainGPT HTTP client."""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Call ChainGPT API")
    parser.add_argument("path", help="Endpoint path, e.g. /api/v1/news")
    parser.add_argument("--method", choices=["GET", "POST"], default="GET")
    parser.add_argument("--base-url", default=os.environ.get("CHAINGPT_BASE_URL", "https://api.chaingpt.org"))
    ...
    api_key = os.environ.get("CHAINGPT_API_KEY")
    if not api_key:
        print("CHAINGPT_API_KEY is required", file=sys.stderr)
        return 2
    ...
    headers = {
        ...
        "User-Agent": "chaingpt-skill/1.0",
    }
    auth_mode = os.environ.get("CHAINGPT_AUTH_MODE", "bearer").lower()
